...
void calculator()
{
  ...
  if (key == '*' && key != NO_KEY)
  { // to make sure key is actually pressed
  ...
  else if (key == '1' || key == '2' || key == '3' || key == '4' || key == '5' || key == '6' || key == '7' || key == '8' || key == '9' || key == '0' && key != NO_KEY)
  {                              // A number is entered
...
{ // clears everything to do next operation
  lcd.clear();
  operand1 = "";
  operand2 = "";
  operatorChar = "";
...
{
  if (operatorChar == "")
  {                  // entering first num till operator is not entered
...
void doorLock()
{
  ...
  {
    if (key == '*')
    { // to clear display, calc variables and to set new user defined password again
      clearDisplayAndVars();
      input = "";
      password = "";
      count = 0;
